...
- **`pagy` must be explicitly `require`d in `lib/model_driven_api.rb`** — `Api::V3::ApplicationController` calls `include Pagy::Backend` at class-body evaluation time, before Bundler's auto-require runs. Without `require "pagy"` in the entrypoint, any rake task or boot that loads the app raises `NameError: uninitialized constant Pagy`. The declared gemspec dependency (`pagy ~> 9.0`) alone is not sufficient.
